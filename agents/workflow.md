...
**正文里不许有 markdown 装饰**——在排版自检那一步。番茄和起点的编辑器不解析 markdown，加粗、分隔线、列表会原样砸在读者眼前。场景分隔符也不许有——换场直接另起一段，靠第一句落时间与地点。**章首不落日历日期戳**（2026-09-22 作者裁，`04-审校/总编裁决记录.md` 第二十节）：第一段接上一章的钩子或情绪落点，时间用相对词（第二天一早、当天夜里）或并进动作里，钟点可写、日历日期不写，日期只进 `01-设定/时间线台账.md`。
